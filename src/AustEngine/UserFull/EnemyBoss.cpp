...
EnemyBoss::EnemyBoss()
{
	Model *pModelEnemyBoss = ModelManager::Get("Tank");

	Texture *pEnemyBossTex = TextureManager::Get("red");
	...
	myModel = new TankModel(pModelEnemyBoss, pShaderObject_constantColor, pEnemyBossTex, LightColor, LightPos);

	myEffect = ParticleEffectManager::Get("FireExplosion");

	SetColliderModel(pModelEnemyBoss, VolumeType::OBB);
...
void EnemyBoss::Initialize(Tank * t, Vect v, TerrainObject * mod, float time)
{
	myTerrain = mod;
	MyTime = time;
...
void EnemyBoss::Alarm0()
{
	...
	{
		firing = true;
		BossBulletFactory::CreateEnemyBullet(Vect(-scale * 2, turretHeight, 0) * ShipRot + ShipPos, ShipRot);
		BossBulletFactory::CreateEnemyBullet(Vect(0, turretHeight, 0) + ShipPos, ShipRot);
		BossBulletFactory::CreateEnemyBullet(Vect(scale * 2, turretHeight, 0) * ShipRot + ShipPos, ShipRot);
...
void EnemyBoss::SceneExit()
{
	...
	EnemyBossFactory::RecycleEnemy(this);
}